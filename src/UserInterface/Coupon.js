...
function Coupon(props) {
    ...
    const handleSubmit = async () => {
      ...
      var result = await postDataImage('coupon/savecoupon', formData)
      if(result.result)
      {
        Swal.fire({
          title: "BESTMEDS",
          ...
          <img src="/category.png" style={{width:50,height:50}}/>
            ...
            <Grid item xs={12}>
              <CssTextField variant="outlined" InputLabelProps={{
                style: { color: '#FFF' },
              }} inputProps={{ style: { color: "#FFF" } }} label="coupon Status" onChange={(event) => setCouponStatus(event.target.value)} fullWidth />
